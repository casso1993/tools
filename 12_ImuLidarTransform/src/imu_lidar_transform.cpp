...
int main() {
    ...
    // 计算四元数 (w, x, y, z)
    double w = cos(imu2lidar_angle_rad / 2.0);
    double x = 0.0;  // 绕Z轴旋转，x和y分量为0
    double y = 0.0;
    double z = sin(imu2lidar_angle_rad / 2.0);
    
    // 计算旋转矩阵（绕Z轴）
    double cos_theta = cos(imu2lidar_angle_rad);
    double sin_theta = sin(imu2lidar_angle_rad);
